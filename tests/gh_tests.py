...
from sources.github_sync import (
    BRANCH_TTL,
    BranchWorker,
    GithubSync,
    HEAD_BASE_SEPARATOR,
)
from sources.patchwork import Series, Subject

from .pw_tests import (
    DEFAULT_TEST_RESPONSES,
    get_default_pw_client,
    pw_response_generator,
)

# TEST_LABELS_CFG pulled from `sources/labels.json`
TEST_LABELS_CFG = {
...
TEST_CI_REPO_URL = f"https://user:pass@127.0.0.1:0/{TEST_CI_ORG}/{TEST_CI_REPO}"
TEST_CI_BRANCH = "test_ci_branch"
TEST_BASE_DIRECTORY = "/repos"
TEST_PW_URL = "pw"
TEST_PW_SARCH_PATTERNS = "pw-search-pattern"
TEST_TAG_TO_BRANCH_MAPPING = "tag-to-branch-mapping"
TEST_BRANCH = "test-branch"
TEST_OAUTH_TOKEN = "test-oauth-token"
TEST_CONFIG = {
    "pw_url": TEST_PW_URL,
    "pw_search_patterns": TEST_PW_SARCH_PATTERNS,
    "branches": {
        TEST_BRANCH: {
            "repo": TEST_REPO,
    ...
    "tag_to_branch_mapping": TEST_TAG_TO_BRANCH_MAPPING,
}
...
def get_default_gh_client(config=TEST_CONFIG) -> GithubSyncMock:
    return GithubSyncMock(config=config)
...
class TestBranchWorker(unittest.TestCase):
    def setUp(self) -> None:
        ...
        patcher = patch("sources.github_sync.git.Repo")
        self._git_repo_mock = patcher.start()
        self.addCleanup(patcher.stop)

        self._bw = get_default_bw_client()
    ...
    def test_fetch_repo_branch(self) -> None:
        ...
        with patch.object(BranchWorker, "fetch_repo") as fr:
            ...
            self.assertIn(
                TEST_CI_BRANCH, self._bw.ci_repo_local.git.checkout.call_args.args[0]
            )
    ...
    def test_do_sync_create_remote(self) -> None:
        ...
        with patch.object(self._bw, "repo_local") as lr:
            ...
            lr.create_remote.assert_called_once_with(
                self._bw.upstream_remote, TEST_UPSTREAM_REPO_URL
            )

            lr.delete_remote.assert_not_called()
            # After having created the remote, we access it.
            lr.remote.assert_called_once_with(self._bw.upstream_remote)
    ...
    def test_do_sync_recreate_remote_when_changed(self) -> None:
        ...
        with patch.object(self._bw, "repo_local") as lr:
            ...
            lr.delete_remote.assert_called_once_with(self._bw.upstream_remote)
            # We access the remote twice.
            self.assertEqual(2, lr.remote.call_count)
            lr.remote.assert_called_with(self._bw.upstream_remote)
    ...
    def test_do_sync_reset_repo(self) -> None:
        with patch.object(self._bw, "repo_local") as lr, patch(
            "sources.github_sync._reset_repo"
        ) as rr:
    ...
    def test_fetch_repo_path_doesnt_exist_full_sync(self) -> None:
        ...
        with patch.object(self._bw, "full_sync") as fr, patch(
            "sources.github_sync.os.path.exists"
        ) as exists:
...
class TestGihubSync(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self._gh = get_default_gh_client()
    ...
    def test_init_with_base_directory(self):
        ...
        for case in test_cases:
            with self.subTest(msg=case.name):
                ...
                gh = get_default_gh_client(config=config)
                self.assertEqual(
                    True,
